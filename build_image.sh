...
#####################
create_ruby_files() {
    # Check if a language ordering is defined
    # it is the order of json files which will dictate
    # their ordering
    if ! [ -f lang_order.sh ]; then
        echo "lang_order.sh does not exist! Must define it to be a proper order of language JSON files!"
        return -1
    fi
    # Defines the LANG_ORDER variable
    source lang_order.sh
    if [ "${#LANG_ORDER[@]}" -eq 0 ]; then
        return -2
    fi
    ...
    for lang_file in "${LANG_ORDER[@]}"; do
...
create_docker_file() {
    ...
    echo "USER judge0" >> Dockerfile
    echo "ENV USER judge0" >> Dockerfile
    echo "ENV USER_HOME_DIR /home/judge0" >> Dockerfile
    echo "COPY ./scripts/prepare_workers /api/scripts/prepare_workers" >> Dockerfile
    echo "COPY ./scripts/workers /api/scripts/workers" >> Dockerfile
...
JUDGE_VERSION="${JUDGE_VERSION:-1.13.1}"
